...
> **Implementation-time amendment (2026-08-11) — authentication removed from §2.1.1:** consumers of this API are trusted, same-cluster internal callers, so no credential (API key, mTLS or OAuth) is presented or verified. `clientId` is taken directly from the request path rather than "derived from the authenticated principal." §2.1.1's claim that authentication is "the load-bearing control preventing one tenant from reading or mutating another's counters, limits or audit records" **no longer holds at the HTTP layer** — any caller may address any `clientId`. What is unchanged: an unknown or non-`ACTIVE` clientId still fails closed before any processing (§2.1.1's second bullet, §4.9), and every data-access query still structurally requires a `clientId` predicate (§2.1.3). The `clients` collection (§2.1.2) no longer carries an authentication-binding field. If this API is ever exposed beyond the trusted cluster, OAuth 2.0 with scoped tokens is the intended mechanism, reintroduced solely at the request's entry point. Full rationale: `docs/stories/STORY-01-02-client-authentication-and-clientid-derivation.md`.
