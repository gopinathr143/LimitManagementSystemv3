...
describe('Dimension registry — STORY-02-01/02/03 (integration, real MongoDB replica set)', () => {
  ...
  test('AC2: PUT rejects a registry missing GLOBAL and names the reason', async () => {
    ...
    const getRes = await request(app).get(`/clients/${clientId}/dimensions`).set('x-api-key', apiKey);
  ...
  test('AC4: a valid PUT persists and GET reflects it with derived window state', async () => {
    const { clientId, apiKey } = await createTestClient(app);
    const putRes = await request(app).put(`/clients/${clientId}/dimensions`).set('x-api-key', apiKey).send({ allowedDimensions: VALID_DIMENSIONS });
  ...
  test('AC1/AC5: two clients enforce only their own registry, independently', async () => {
    ...
    await request(app)
      ...
      .send({ allowedDimensions: VALID_DIMENSIONS });
    await request(app)
    ...
    const aView = await request(app).get(`/clients/${clientA.clientId}/dimensions`).set('x-api-key', clientA.apiKey);
    const bView = await request(app).get(`/clients/${clientB.clientId}/dimensions`).set('x-api-key', clientB.apiKey);
    ...
    await request(app)
      ...
      .set('x-api-key', clientA.apiKey)
      .send({ allowedDimensions: VALID_DIMENSIONS });
    const bViewAgain = await request(app).get(`/clients/${clientB.clientId}/dimensions`).set('x-api-key', clientB.apiKey);
    assert.equal(bViewAgain.body.data.configVersion, 1);
  });

  test('cross-tenant: client A cannot read or write client B\'s registry', async () => {
    const clientA = await createTestClient(app);
    const clientB = await createTestClient(app);
    await request(app)
      .put(`/clients/${clientB.clientId}/dimensions`)
    ...
    const getRes = await request(app).get(`/clients/${clientB.clientId}/dimensions`).set('x-api-key', clientA.apiKey);
  ...
  test('admin credentials cannot access the tenant-scoped dimensions route (wrong credential space)', async () => {
    const { clientId } = await createTestClient(app);
    const res = await request(app).get(`/clients/${clientId}/dimensions`).set('x-admin-api-key', 'it-admin-key-1');
