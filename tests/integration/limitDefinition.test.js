...
describe('Limit definitions — STORY-02-04/02-05 (integration, real MongoDB replica set)', () => {
  ...
  test('DELETE soft-deactivates rather than removing the document', async () => {
    ...
    const stillReadable = await request(app).get(`/clients/${clientId}/limits/${createRes.body.data._id}`);
    assert.equal(stillReadable.status, 200);
  });

  test('two clients\' limit definitions stay independent (structural isolation, not credential-based)', async () => {
    ...
    assert.equal(aList.body.data[0].thresholdAmount, 1000);
    assert.equal(bList.body.data[0].thresholdAmount, 2000);
  });

  test('STORY-01-04: an unregistered clientId is rejected before any limits access', async () => {
    const res = await request(app).get('/clients/NO_SUCH_CLIENT/limits');
